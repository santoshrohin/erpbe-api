 ...
 *
 * Run: npx ts-node scripts/check-coverage-parity.ts
 * Or:  node --require ts-node/register scripts/check-coverage-parity.ts
 */
...
// Registered modules — each must have UI + API + DB coverage
...
function readSpecFiles(moduleDir: string): string {
  const pattern = path.join(TESTS_DIR, moduleDir, '*.spec.ts');
  const files = glob.sync(pattern);
  return files.map(f => fs.readFileSync(f, 'utf8')).join('\n');
}

function checkCoverage(): ModuleCoverage[] {
  return MODULES.map(mod => {
    ...
    const hasDB = /\bdb\.(queryOne|query)\b|getLatest|getPoBy|getTaxInvoice|getChallan|getStockEntries/.test(content);

    return { name: mod.name, dir: mod.dir, hasUI, hasAPI, hasDB };
  });
}
...
function main(): void {
  ...
  let anyFailed = false;
  const lines: string[] = ['', '=== Coverage Parity Report ===', ''];
