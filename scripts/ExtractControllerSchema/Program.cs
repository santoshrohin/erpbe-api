...
namespace ExtractControllerSchema
{
    class Program
    {
        ...
        static string GenerateCreateTableScript(SqlConnection connection, string schemaName, string tableName)
        {
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        if (string.IsNullOrEmpty(pkIndexName))
                            pkIndexName = reader["IndexName"].ToString() ?? "";
                        string colName = reader["ColumnName"].ToString() ?? "";
            ...
            if (pkColumns.Count > 0 && !string.IsNullOrEmpty(pkIndexName))
            {
                sb.AppendLine($"    ALTER TABLE [{schemaName}].[{tableName}]");
                sb.AppendLine($"    ADD CONSTRAINT [{pkIndexName}] PRIMARY KEY ({string.Join(", ", pkColumns)});");
            }
