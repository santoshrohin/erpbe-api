-- =============================================
-- Tax Invoice Print Data - V2 (CORRECT FORMAT)
...
BEGIN
    ...
    DECLARE @CompanyState NVARCHAR(100);
    DECLARE @CustomerState NVARCHAR(100);
    ...
    SELECT 
        ...
        CASE WHEN ISNULL(CM_STATE, '') <> '' THEN ', ' + CM_STATE ELSE '' END +
    ...
    SELECT 
        ...
        ISNULL(@CompanyState, 'Maharashtra') AS PlaceOfSupply
    FROM INVOICE_MASTER
    ...
    SELECT 
        ...
        -- Build address
        ISNULL(P_ADD1, '') + 
        CASE WHEN ISNULL(P_ADD2, '') <> '' THEN ',' + P_ADD2 ELSE '' END +
        CASE WHEN ISNULL(P_CITY, '') <> '' THEN ',Dist-' + P_CITY ELSE '' END +
        CASE WHEN ISNULL(P_PIN_CODE, '') <> '' THEN '-' + P_PIN_CODE ELSE '' END AS Address,
        ISNULL(P_STATE, '') AS StateName,
        -- Get state code using CASE statement
        CASE UPPER(LTRIM(RTRIM(ISNULL(P_STATE, ''))))
    ...
    FROM PARTY_MASTER
    WHERE P_CODE = @CustomerCode
    AND ISNULL(ES_DELETE, 0) = 0;
    
    -- ============================================
    -- Result Set 4: Delivery Details (Right side)
    -- Usually same as recipient, but can be different for shipping
    -- ============================================
    SELECT 
        ISNULL(P_NAME, '') AS Name,
        ISNULL(P_ADD1, '') + 
        ...
        ISNULL(P_GST_NO, '') AS GstinNo
    FROM PARTY_MASTER
    WHERE P_CODE = @CustomerCode  -- Could join to shipping address if different
    AND ISNULL(ES_DELETE, 0) = 0;
    ...
    -- ============================================
    SELECT 
        ROW_NUMBER() OVER (ORDER BY IND.IND_CODE) AS SrNo,
